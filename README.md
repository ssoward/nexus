...
- **Mobile-friendly** — full-screen opaque sidebar overlay on mobile (hamburger opens a solid panel covering the entire viewport, no bleed-through); mobile detection uses `(pointer: coarse)` so touch devices are correctly identified regardless of reported viewport width or iOS "Request Desktop Site" mode; soft keyboard support (tested on iOS Safari / Chrome Android); quick-access keybar with Tab, ^C, Paste, arrows, and common Ctrl combos; **Mic** button for voice-to-text input when the browser supports the Web Speech API; Enter and all control sequences bypass xterm.js bracketed-paste wrapping so readline-based apps (Claude Code, bash, etc.) receive bare `\r` and respond correctly; uses `h-dvh` (dynamic viewport height) so the terminal fills the full visible area on iOS Safari regardless of browser chrome or software keyboard; terminal column/row counts are measured only after custom fonts load (`document.fonts.ready`) so xterm never inherits the wrong cell width from a fallback font; mobile font is 10px (~65 columns on a 390pt screen) so Claude Code TUI output fits without mid-word wrapping; flex height chain uses `flex flex-col min-h-0` on the grid wrapper and `h-full` on TerminalPane so the terminal fills the full viewport height with no empty space below when the keyboard is hidden
- **PWA re-auth on every open** — when added to the iOS/Android home screen, the app requires Face ID / Touch ID on every launch; a `sessionStorage` stamp is set at login and checked on startup — if absent (cleared by the OS when the PWA is closed), the server cookie is revoked and the login screen is shown; page refreshes within an active session are unaffected
- **In-app reload button** — circular-arrow button in the header lets PWA users force-refresh the app to pick up new deployments without needing browser pull-to-reload
- **Mobile Pages overlay** — selecting an embedded page on mobile opens it full-screen (z-50 overlay) with a back-arrow to return to terminals and an "Open in browser" link; the split-view panel is reserved for desktop
