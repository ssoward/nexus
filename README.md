...
- **Long-lived sessions** — the auth cookie persists across app/browser restarts (7-day TTL with a 7-day absolute ceiling), so reopening the PWA does not require re-login until the session expires
...
| **Icon is red but the stack is actually healthy** | When the icon isn't green the app logs the offending component to `~/.nexus/logs/menubar.err.log` (e.g. `overall=down \| … tailscale=down(no route) …`) — check there first. Two known causes: **(1) colima** — `colima status` does a `PATH` lookup for its `limactl`/`docker` dependencies, so the `com.nexus.menubar` agent's `PATH` must include `/opt/homebrew/bin` (verify with `launchctl print gui/$(id -u)/com.nexus.menubar \| grep -A3 EnvironmentVariables`). **(2) tailscale** — the Mac App Store Tailscale CLI talks to the GUI app and fails (`CLIError error 3`) when spawned directly from the background agent; the probe works around this by invoking it through a login shell (`/bin/bash -lc`). Either way, re-run `bash macos/install.sh` to rebuild the app + re-render the plist, then re-login. |
...
session:
  ...
  jwt_expire_minutes: 10080     # 7 days (matches the 7-day absolute session ceiling)
